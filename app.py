...
def detect_wells(rgb, s):
    ...
    c=cv2.HoughCircles(blur,cv2.HOUGH_GRADIENT,dp=1.15,minDist=s.well_spacing,param1=75,param2=s.hough_p2,minRadius=s.well_rmin,maxRadius=s.well_rmax)
    if c is None: return np.empty((0,3),int)
    c=np.round(c[0]).astype(int)
    kept=[]
    for x,y,r in c[np.argsort(c[:,0])]:
        if all((x-a)**2+(y-b)**2>20**2 for a,b,_ in kept): kept.append((x,y,r))
...
def segment_pdos(green,s):
    labs=label(green>s.green_low)
    out=[]
    for reg in regionprops(labs,intensity_image=green):
        if reg.area<s.pdo_min_area or reg.intensity_max<s.green_high: continue
        y0,x0,y1,x1=reg.bbox
        mask=labs[y0:y1,x0:x1]==reg.label
        sub=green[y0:y1,x0:x1]
        pieces=None
        if s.split_pdos:
            peaks=peak_local_max(sub,min_distance=s.pdo_peak_distance,threshold_abs=s.green_high,labels=mask.astype(np.uint8),exclude_border=False)
            if len(peaks)>=2:
                ...
                cand=[p for p in regionprops(ws,intensity_image=sub) if p.area>=s.pdo_min_area and p.intensity_max>=s.green_high]
            ...
            if pieces is None:
                ...
                if len(sp)==2:
                    ...
                    if min(vals)/max(vals)>=0.60 and sep>=18:
                        ...
                        cand=[p for p in regionprops(ws,intensity_image=sub) if p.area>=max(220,s.pdo_min_area) and p.intensity_max>=s.green_high]
                        if len(cand)==2:
...
def detect_psc(rgb,x,y,r,s):
    ...
    x0=max(0,x-R); x1=min(rgb.shape[1],x+R+1); y0=max(0,y-R); y1=min(rgb.shape[0],y+R+1)
    ...
    score[~mask]=-999
    peaks=peak_local_max(score,min_distance=s.psc_peak_distance,threshold_abs=s.psc_peak_threshold,exclude_border=False)
    f=[]
    for py,px in peaks:
        if sub[py,px,0]-sub[py,px,2]>=s.psc_red_minimum:
...
def crop_square(rgb,x,y,r,scale=4):
    ...
    side=max(im.size); c=Image.new('RGB',(side,side),'black'); c.paste(im,((side-im.width)//2,(side-im.height)//2))
    return c.resize((side*scale,side*scale),Image.Resampling.NEAREST)


def labelled_crop(crop,series,well,pdo_n,psc_n,sizes):
    title,body=fonts()
    lines=[f'Image {series:02d} | Well {well}', f'PDO count: {pdo_n} | PSC count: {psc_n}', 'PDO size' + ('s' if len(sizes)!=1 else '') + ': ' + ', '.join(f'{v:.1f} µm' for v in sizes)]
    ...
    header=105; W=max(crop.width,max(widths)+28)
    out=Image.new('RGB',(W,header+crop.height),'white'); out.paste(crop,((W-crop.width)//2,header)); dr=ImageDraw.Draw(out); dr.rectangle((0,0,W,header),fill='black')
    y=8
...
def indexed_overlay(rgb,wells,series):
...
def process(files,s,cols):
    ...
    for idx,p in enumerate(paths,1):
        ...
        circles=np.asarray([c for c in circles if c[0]-c[2]>=2 and c[0]+c[2]<W-2 and c[1]-c[2]>=2 and c[1]+c[2]<H-2],int)
        if len(circles)==0: raise RuntimeError(f'No fully visible wells detected in {p.name}.')
        xs=cluster(circles[:,0]); ys=cluster(circles[:,1]); umpp=s.well_diameter_um/(2*np.median(circles[:,2]))
        pdos=segment_pdos(green_excess(rgb),s)
        local=[]
        for x,y,r in circles:
            col,row=grid_index(x,y,xs,ys); well=f'{col},{row}'; assigned=[]
            for obj in pdos:
                if (obj['x']-x)**2+(obj['y']-y)**2 <= (0.86*r)**2: assigned.append(obj)
            psc=detect_psc(rgb,x,y,r,s); sizes=[2*math.sqrt(o['area']/math.pi)*umpp for o in assigned]
            local.append({'x':int(x),'y':int(y),'r':int(r),'well':well,'col':col,'row':row,'pdo_n':len(assigned),'psc_n':len(psc),'sizes':sizes})
            ...
            for n,(obj,size) in enumerate(zip(assigned,sizes),1):
                pdo_rows.append({'image_series':series,'well_index':well,'PDO_number_in_well':n,'PDO_count_in_well':len(assigned),'centroid_x_px':obj['x'],'centroid_y_px':obj['y'],'projected_area_px2':obj['area'],'equivalent_circular_diameter_um':size,'PSC_like_focus_count_in_well':len(psc)})
            if assigned:
        ...
        indexed_overlay(rgb,local,series).save(out/'indexed_large_images'/f'series_{series:02d}_indexed.png',dpi=(300,300))
    ...
    wdf=pd.DataFrame(wells_rows); pdf=pd.DataFrame(pdo_rows); idf=pd.DataFrame(image_rows)
    wdf.to_csv(out/'csv'/'well_raw_data.csv',index=False); pdf.to_csv(out/'csv'/'PDO_raw_data.csv',index=False); idf.to_csv(out/'csv'/'image_summary.csv',index=False)
    if len(pdf):
        d=pdf['equivalent_circular_diameter_um'].astype(float); mean=float(d.mean()); sd=float(d.std(ddof=1)) if len(d)>1 else 0.0
        fig,ax=plt.subplots(figsize=(6.2,4.6)); ax.hist(d,bins=s.histogram_bins,edgecolor='black'); ax.axvline(mean,ls='--',label=f'Mean = {mean:.1f} µm'); ax.axvline(mean-sd,ls=':',label=f'±1 SD = {sd:.1f} µm'); ax.axvline(mean+sd,ls=':'); ax.set(xlabel='PDO equivalent circular diameter (µm)',ylabel='Number of PDOs'); ax.legend(frameon=False); fig.tight_layout(); fig.savefig(out/'figures'/'PDO_size_distribution.png',dpi=300); plt.close(fig)
    ...
    if labelled_paths: make_contact(labelled_paths,out/'figures'/'PDO_well_contact_sheet_compact.png',cols=cols)
...
with st.sidebar:
    ...
    with st.expander('Advanced thresholds'):
        rmin=st.number_input('Minimum well radius (px)',5,200,23); rmax=st.number_input('Maximum well radius (px)',6,300,40); spacing=st.number_input('Minimum well spacing (px)',10,500,54); hp2=st.number_input('Well detection sensitivity',1.0,100.0,27.0,1.0)
        gl=st.number_input('PDO green low threshold',0.0,255.0,30.0,1.0); gh=st.number_input('PDO green high threshold',0.0,255.0,45.0,1.0); amin=st.number_input('Minimum PDO area (px²)',1,100000,20); pd=st.number_input('PDO split peak distance (px)',1,100,18)
        pt=st.number_input('PSC focus threshold',0.0,255.0,9.0,0.5); prm=st.number_input('PSC red-minus-blue minimum',0.0,255.0,12.0,0.5); ppd=st.number_input('PSC focus minimum spacing (px)',1,100,4)
run=st.button('Run analysis',type='primary',use_container_width=True,disabled=not files)
if run:
    s=Settings(well,float(rmin),float(rmax),int(spacing),float(hp2),float(gl),float(gh),int(amin),bool(split),int(pd),float(pt),float(prm),int(ppd),int(bins))
    bar=st.progress(5,text='Starting analysis…')
    try:
        root,out,summary,idf=process(files,s,int(cols)); bar.progress(100,text='Complete')
        st.session_state['zip']=zip_bytes(out); st.session_state['out']=str(out); st.session_state['summary']=summary.to_dict('records'); st.session_state['idf']=idf.to_dict('records'); st.success('Analysis complete.')
