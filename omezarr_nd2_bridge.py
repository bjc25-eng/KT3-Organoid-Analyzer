...
DETECTOR_SCHEMA_VERSION = 'nd2-omezarr-array-footprint-hough-v6'

# Tile-level array detection only. Once an array footprint is found, Hough well
# detection itself remains unchanged from the successful single-array workflow.
        ...
        try:
            ...
            if omero_channels:
                payload['channel_metadata'] = [
                    {'index': int(i), 'name': str(row.get('label', f'Channel {i}')),
                     'color': row.get('color'), 'window': row.get('window')}
...
def install_omezarr_window_scaled_reads() -> None:
    ...
    def wrapped(self, x0: int, y0: int, width: int, height: int, channel: int = 0,
        ...
        x0_i = max(0, int(x0)); y0_i = max(0, int(y0))
        ...
        return np.clip((work-low) * (255.0/(high-low)), 0.0, 255.0).astype(np.uint8)
...
def _well_like_closed_contours(rgb: np.ndarray, settings) -> list[dict]:
    """Find several closed structures compatible with the physical well size."""
...
def _array_footprint_mask(rgb: np.ndarray, settings) -> tuple[np.ndarray, dict]:
    """Build the spatial footprint of the array inside a mixed array/background tile.

    Closed well-like contours seed the footprint. Their convex hull is expanded by
    one physical well-centre spacing so boundary wells with imperfect contours are
    still included, while distant background in the same 1024-px tile is excluded.
    """
    ...
    # Geometry-derived padding: one expected centre spacing. This keeps one row
    # of boundary wells around the contour-supported core without admitting an
    # entire blank half-tile.
    pad = max(int(round(float(settings.well_spacing))), int(round(float(settings.well_rmax))))
    kernel_size = 2*pad + 1
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    mask = cv2.dilate(mask, kernel, iterations=1)
    qc['array_footprint_fraction'] = float(np.mean(mask > 0))
...
def _hough_candidates_converted_dic(rgb: np.ndarray, settings) -> np.ndarray:
    ...
    circles = cv2.HoughCircles(
        ...
        param1=75.0, param2=float(settings.hough_p2),
...
def detect_wells_converted_dic_with_qc(rgb: np.ndarray, settings) -> tuple[np.ndarray, list[dict]]:
    """Detect as before, but retain centres only inside the local array footprint."""
    ...
    for x, y, r in candidates:
        inside = 0 <= int(x) < w and 0 <= int(y) < h and footprint[int(y), int(x)] > 0
        rows.append({
            'x': int(x), 'y': int(y), 'detected_radius_px': int(r),
            'inside_array_footprint': bool(inside),
            'accepted_by_converted_detector': bool(inside),
            **tile_qc,
        })
        if inside:
            accepted.append((int(x), int(y), int(r)))
...
def process_converted_nd2_omezarr_qc(*args, **kwargs):
    ...
    if config_path.exists():
        try:
            ...
            final_qc['physical_pixel_size_source'] = 'OME-NGFF X/Y coordinateTransformations preserved from original Nikon ND2'
            final_qc['analysis_source'] = 'ND2-derived OME-Zarr'
            final_qc['channel_scaling'] = 'per-channel OME display windows mapped to uint8 before image processing'
            final_qc['channel_scaling_schema'] = SCALING_SCHEMA_VERSION
            final_qc['well_detector'] = 'successful local-contrast physical-radius Hough detector restricted to a contour-derived local array footprint'
